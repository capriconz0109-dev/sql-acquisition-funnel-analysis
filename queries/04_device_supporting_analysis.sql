...
aggregated AS (
  ...
  FROM session_funnel
  WHERE (traffic_source = 'google' AND traffic_medium IN ('organic', 'cpc'))
     OR (traffic_source = '(direct)' AND traffic_medium = '(none)')
     OR (traffic_source = 'shop.googlemerchandisestore.com' AND traffic_medium = 'referral')
  GROUP BY 1, 2, 3
)
...
ORDER BY traffic_source, traffic_medium, sessions DESC;
